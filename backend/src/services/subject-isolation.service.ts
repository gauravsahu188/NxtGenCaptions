...
export class SubjectIsolationService {
  ...
  async isolateSubject(videoInput: string, userId = "anonymous"): Promise<string> {
    ...
    if (videoInput.startsWith("http://localhost:") || videoInput.startsWith("http://127.0.0.1:")) {
      filename = path.basename(videoInput.split("?")[0]);
      localVideoPath = path.join(process.cwd(), "uploads", filename);
    } else if (fs.existsSync(videoInput)) {
      ...
      const matched = fs.readdirSync(uploadsDir).find(f => f.includes(filename) || filename.includes(f));
      if (matched) {
        localVideoPath = path.join(uploadsDir, matched);
        filename = matched;
      }
    }
    ...
    const outputLocalPath = path.join(process.cwd(), "uploads", outputFilename);
    ...
    if (replicateToken) {
      try {
        ...
        if (localVideoPath && fs.existsSync(localVideoPath) && !videoInput.startsWith("https://")) {
        ...
        if (data.id) {
          ...
          if (resultUrl) {
            ...
            fs.writeFileSync(outputLocalPath, Buffer.from(arrayBuffer));
    ...
    return new Promise((resolve) => {
      ...
      try {
        fs.copyFileSync(localVideoPath, outputLocalPath);
