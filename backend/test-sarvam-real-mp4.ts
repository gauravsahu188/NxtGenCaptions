...
import { SarvamTranscriptionService } from "./src/services/sarvam.service";
import { FFmpegService } from "./src/services/ffmpeg.service";
import fs from "fs";

async function run() {
  console.log("Using API Key:", process.env.SARVAM_API_KEY ? "Set" : "Not Set");
  
  const videoPath = path.join(__dirname, "uploads", "video-1778788113973-761083402.mp4");
  const audioFilename = "temp_test_audio.mp3";
  const audioPath = path.join(__dirname, "temp", audioFilename);
  
  const ffmpegService = new FFmpegService();
  const service = new SarvamTranscriptionService();
  ...
  try {
    ...
    const duration = await ffmpegService.getVideoDuration(videoPath);
    console.log("Video duration:", duration);
    
    console.log("Transcribing...");
    
    // We can spy on the raw response by monkeypatching the fetch or just printing inside the service.
    // For now, let's read the file directly or let's inspect the transcription result.
    const res = await service.transcribeAudio(extractedAudio, (seg) => {
      // console.log("OnProgress segment:", JSON.stringify(seg));
    }, { language: "hi", script: "native", duration });
